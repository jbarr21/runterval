...
sealed class State {
  ...
  sealed class WorkingOut : State() {
    abstract val workout: Workout
    abstract val remaining: Duration
    abstract val paused: Boolean
    abstract val name: CharSequence
    abstract val duration: Duration
    abstract fun togglePause(paused: Boolean): WorkingOut
    abstract fun reset(): WorkingOut

    data class WarmingUp(
        override val workout: Workout,
        override val remaining: Duration = workout.warmup,
        override val paused: Boolean = true) : WorkingOut() {

      override val name = "Warmup"
      override val duration = workout.warmup
    ...
    data class Intervals(
        ...
        override val paused: Boolean = false) : WorkingOut() {
    ...
    data class CoolingDown(
      ...
      override val duration = workout.cooldown
      override fun togglePause(paused: Boolean) = copy(paused = paused)
